...
VisionComm::VisionComm(ros::Publisher * pub) : pub_(pub) {
 ...
 */
void VisionComm::updateInfo(const SSL_DetectionRobot& robot,
		int detectedTeamColor) {
	...
	if (robot.has_robot_id()) {
		...
		msg.pose.position.x = robot.x();
		msg.pose.position.y = robot.y();
		msg.pose.orientation = tf::createQuaternionMsgFromYaw (robot.orientation());
 ...
 */
void VisionComm::recieveRobotTeam(const SSL_DetectionFrame& frame,
		int whichTeam) {
	//for(const SSL_DetectionRobot& robot : frame.robots_blue().iterator)//*currentTeamDection changed to robot_blue AE
	for (google::protobuf::RepeatedPtrField<SSL_DetectionRobot>::const_iterator it =
			frame.robots_blue().begin(); it != frame.robots_blue().end();
			it++) {
